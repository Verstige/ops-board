/**
 * Vendor mutation proxy. Forwards PATCH + DELETE to /api/vendors/:id
...
export async function PATCH(
  ...
  if (!base) {
    return NextResponse.json({ error: "OPEN_LOCAL_API_URL not set" }, { status: 503 });
  }

  try {
    const url = `${base}/api/vendors/${id}`;
